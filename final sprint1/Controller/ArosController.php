...
/**
 * localhost/sprint1/users/admin/controller_index method
 *
 * @return void
 */
	public function localhost/sprint1/users/admin/controller_index() {
		$this->Aro->recursive = 0;
		$this->set('aros', $this->Paginator->paginate());
	}

/**
 * localhost/sprint1/users/admin/controller_view method
 *
 * @throws NotFoundException
 * @param string $id
 * @return void
 */
	public function localhost/sprint1/users/admin/controller_view($id = null) {
		if (!$this->Aro->exists($id)) {
			throw new NotFoundException(__('Invalid aro'));
		}
		$options = array('conditions' => array('Aro.' . $this->Aro->primaryKey => $id));
...
/**
 * localhost/sprint1/users/admin/controller_add method
 *
 * @return void
 */
	public function localhost/sprint1/users/admin/controller_add() {
		if ($this->request->is('post')) {
			$this->Aro->create();
			if ($this->Aro->save($this->request->data)) {
				$this->Session->setFlash(__('The aro has been saved.'));
...
/**
 * localhost/sprint1/users/admin/controller_edit method
 *
 * @throws NotFoundException
 * @param string $id
 * @return void
 */
	public function localhost/sprint1/users/admin/controller_edit($id = null) {
		if (!$this->Aro->exists($id)) {
			throw new NotFoundException(__('Invalid aro'));
		}
		if ($this->request->is(array('post', 'put'))) {
...
/**
 * localhost/sprint1/users/admin/controller_delete method
 *
 * @throws NotFoundException
 * @param string $id
 * @return void
 */
	public function localhost/sprint1/users/admin/controller_delete($id = null) {
		$this->Aro->id = $id;
		if (!$this->Aro->exists()) {
			throw new NotFoundException(__('Invalid aro'));
		}
